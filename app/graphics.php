...
if (isset($_REQUEST['width']) && isset($_REQUEST['height']))
{
    ...
    {
        $bk_color = json_decode($_REQUEST['background']);
        $bk = imagecolorallocate($img, $bk_color[0], $bk_color[1], $bk_color[2]);
    } else $bk = imagecolorallocate($img, 200, 200, 200);
    ...
    foreach($shapes as $shape)
    {
      ...
      {   
        ...
        for($i=0;$i<$num_points;$i++)
        {
            if ($shape->length_labels[$i])
            {
                ...
                {
                  imagestring($img,5, $mid_len['x'],$mid_len['y'], ((int)$mid_len['len']).$shape->length_label_unit, $pen);
                } else
                {
                  imagestring($img,5, $mid_len['x'],$mid_len['y'], $shape->length_labels[$i], $pen);
                }
            }
        }
      ...
      {   
        ...
        {
          imagestring($img,5, $mid_len['x'],$mid_len['y'], $shape->length_labels[0], $pen);
        }
        
      }
      else if (strtolower(trim($shape->name)) == 'text')
      {
          $white = imagecolorallocate($img, 255, 255, 255);
          $pen = imagecolorallocate($img,$shape->color[0], $shape->color[1], $shape->color[2]);
          imagestring($img,5, $shape->vertices[0],$shape->vertices[1], $shape->text, $pen);
      }
      else if (strtolower(trim($shape->name)) === 'ellipse')
      {   
        ## colors
        $white = imagecolorallocate($img, 255, 255, 255);
        $pen = imagecolorallocate($img,$shape->color[0], $shape->color[1], $shape->color[2]);
        imageellipse( $img , $shape->vertices[0], $shape->vertices[1] , $shape->width ,$shape->height , $pen );
        
      }
      else if (strtolower(trim($shape->name)) === 'filled-ellipse')
      {   
        ## colors
        $white = imagecolorallocate($img, 255, 255, 255);
        $pen = imagecolorallocate($img,$shape->color[0], $shape->color[1], $shape->color[2]);
        imagefilledellipse( $img , $shape->vertices[0], $shape->vertices[1] , $shape->width ,$shape->height , $pen );
