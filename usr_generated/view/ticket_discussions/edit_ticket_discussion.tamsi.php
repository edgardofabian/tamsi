...
{
	$role->showUnauthorizedMessage($allowed,"?command=display_php&base_path=usr/view&path=Edit_TicketDiscussion.rest.php");
} else
{
...
if ($ticket_discussion_view->title)
{
    echo '<h2>'.$ticket_discussion_view->title.'</h2>';
}
?>



<form id="edit_ticket_discussion" <?php echo $ticket_discussion_view->form_attributes;?> class="edit" action="?command=edit&object=TicketDiscussion&base_path=<?php echo $_REQUEST["base_path"];?>&path=<?php echo $path;?>" method="POST">
...
<thead><th><?php echo $ticket_discussion_view->field_header;?></th><th><?php echo $ticket_discussion_view->field_value_header;?><?php if ($role->isUserAnyOf($allowed_edit)) {?><button type='button' id='toggle_edit' class='button mx-'  mx-click='toggleEdit()'><li class='fa fa-pencil'></li> Enable Edit</button> <?php }?></th></thead>
<tbody>
<?php
foreach($ticket_discussion_view->edit_columns as $field)
{
    $label = $ticket_discussion_view->columns[$field]['label'];
    ...
    if ($field=='id')
    {?>
        <tr id='tr_id' ><td class='label'><?php echo $label;?></td><td id='td_id' ><input type='number' style='display:none' name='id' value='<?php echo $ticket_discussion_view->ticket_discussion->getId();?>' ></input><?php echo $ticket_discussion_view->ticket_discussion->getId();?></td></tr>
    <?php
