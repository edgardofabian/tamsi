...
{
	$role->showUnauthorizedMessage($allowed,"?command=display_php&base_path=usr/view&path=New_Ticket.rest.php");
} else
{
...
<form id="new_ticket" <?php echo $ticket_view->form_attributes;?> class="new" action="?command=new&object=Ticket&base_path=<?php echo $_REQUEST["base_path"];?>&path=<?php echo $path;?>" method="POST">
<input class="field_pages" field_pages="1" type="hidden" value="1" name="pages" />
<?php $ticket_view->renderCsrf();?> 
<?php $ticket_view->renderCustomNew();?> 
<?php if ($role->isUserAnyOf($allowed_edit)) { $ticket_view->renderNewControls(); }?>
<table class='odd_even_row_alternate_color full_width data' >
<thead><th><?php echo $ticket_view->field_header;?></th><th><?php echo $ticket_view->field_value_header;?></th></thead>
<tbody>
<?php
foreach($ticket_view->new_columns as $field)
{
    $label = $field;
