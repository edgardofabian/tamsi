...
{
	$role->showUnauthorizedMessage($allowed,"?command=display_php&base_path=usr/view&path=New_Configuration.rest.php");
} else
{
...
<form id="new_configuration" <?php echo $configuration_view->form_attributes;?> class="new" action="?command=new&object=Configuration&base_path=<?php echo $_REQUEST["base_path"];?>&path=<?php echo $path;?>" method="POST">
...
<?php if ($role->isUserAnyOf($allowed_edit)) { $configuration_view->renderNewControls(); }?>
<table class='odd_even_row_alternate_color full_width data' >
<thead><th><?php echo $configuration_view->field_header;?></th><th><?php echo $configuration_view->field_value_header;?></th></thead>
<tbody>
<?php
foreach($configuration_view->new_columns as $field)
{
    $label = $field;
