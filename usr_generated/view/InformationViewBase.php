...
class InformationViewBase extends ViewBase 
{ 
    public $id;
    public $security;
    public $current_user;
    ...
    public $sort_filters;
    public $sort_orders;
    public function get_user_id($information)
    {
         echo $information->getUser();
    }
    public function get_name($information)
    {
        echo $information->getName();
    }
    public function get_value($information)
    {
        echo $information->getValue();
    }
    public function get_token($information)
    {
        echo $information->getToken();
    }
    public function get_salt($information)
    {
        echo $information->getSalt();
    }
    public function get_active($information)
    {
        echo $information->getActive();
    }
    public function get_created($information)
    {
    ...
    public function render_user_id()
    {
        ...
        else if (($this->columns[$information_field]['type']=='hidden') || ($this->columns[$information_field]['type']=='readonly'))
        {
            ...
            if ($this->columns[$information_field]['type']=='readonly')
            {
                echo "<span >".$this->information->getUser()."</span>";
            }
        } else
        {
            if ($this->columns[$information_field]['mode']=='edit')
    ...
    {
		echo "<select  class='filter' name='informations_select_filters[active]' value='' > \r\n";
   ...
   public function __construct($param=array('container'=>'div.page'))
   {
      ...
      $this->current_user->find(array("id"=>$_SESSION["user_id"]));
        ...
        $this->headers = array("#"=>array("label"=>"#","sort"=>0,"class"=>"","attributes"=>array()),"action"=>array("label"=>"Action","sort"=>0,"class"=>"","attributes"=>array()),"id"=>array("label"=>"Id ","sort"=>1,"search"=>1,"class"=>"","attributes"=>array()),"user_id"=>array("label"=>"Users ","sort"=>1,"search"=>1,"class"=>"","attributes"=>array()),"name"=>array("label"=>"Name ","sort"=>1,"search"=>1,"class"=>"","attributes"=>array()),"value"=>array("label"=>"Value ","sort"=>"0","search"=>"0","class"=>"","attributes"=>array()),"token"=>array("label"=>"Token ","sort"=>"0","search"=>"0","class"=>"","attributes"=>array()),"salt"=>array("label"=>"Salt ","sort"=>1,"search"=>1,"class"=>"","attributes"=>array()),"active"=>array("label"=>"Active ","sort"=>1,"search"=>1,"class"=>"","attributes"=>array()),"created"=>array("label"=>"Created ","sort"=>1,"search"=>1,"class"=>"","attributes"=>array()));
   } 
   ...
   public function setLabels($labels)
   {
    ...
    { 
        ...
        $this->button_next_url='?command=display_rest&path=Admin.dir/Informations.dir/Edit_Information.hid.rest.php&id='.$this->information->getId(); //url that will be visited after completing the edit submission
        $this->delete_button_url='?command=deleteRest&object=Information'; //url of the controller where delete form will be submitted
        $this->delete_button_next_url='?command=display_rest&path=Admin.dir/Informations.dir/List_Informations.rest.php'; //url that will be visited after completing the delete submission
    ...
    { 
        ...
        $this->button_next_url='?command=display_rest&path=Admin.dir/Informations.dir/List_Informations.rest.php'; //url of the controller that will be visited after completing the new submission
    ...
    { 
        ...
        $this->command = 'list';
        $this->list_controls = array('new'=>array('label'=>'New','url'=>'?command=display_rest&path=Admin.dir/Informations.dir/New_Information.rest.php','container'=>$this->container));
		$this->form_list='form_list_informations';
		$this->action='?command=display_php&path=Admin.dir/Informations.dir/List_Informations.rest.php';
        ...
        $this->list_delete_button_next_url = '?command=display_rest&base_path=Admin.dir/Informations.dir&path=List_Informations.rest.php';
    ...
    public function renderDeleteCheckbox($information)
    {
        echo '<input type="checkbox" class="select_checkbox" id="'.$information->getId().'" value="'.$information->getId().'" name="delete_selected[]" /><br>';
    }
    
    public function renderEditAction($information)
    {
        echo "<button type='button' class='mx- $class' mx-container='".$this->container."' mx-click='?command=display_rest&base_path=Admin.dir/Informations.dir&path=Edit_Information.hid.rest.php&id=".$information->getId()."&object=Information' href='#' ><li class='fa fa-pencil'></li> edit</button>";
    }
    ...
    public function renderDeleteAction($information,$next_url,$delete_url)
    {
        $mx_next_url='';
        if ($next_url)
        {
        ...
        echo '<button type="button" class="button mx- danger" mx-confirm="Are you sure to delete Information of id='.$information->getId().'?" mx-click="'.$delete_url.'&id='.$information->getId().'" href="#" mx-container="'.$this->container.'" '.$mx_next_url.' ><li class="fa fa-trash" ></li> del</button>';
    }
    
    public function renderViewAction($information)
    {
        echo "<button type='button' class='mx- $class' mx-container='".$this->container."' mx-click='?command=display_rest&base_path=Admin.dir/Informations.dir&path=View_Information.hid.rest.php&id=".$information->getId()."&object=Information' href='#' ><li class='fa fa-eye'></li> view</button>";
    }

    public function renderListAction()
    {
        echo "<button type='button' class='button mx- $class' mx-container='".$this->container."' mx-click='?command=display_rest&base_path=Admin.dir/Informations.dir&path=List_Informations.rest.php&object=Information'  ><li class='fa fa-list'></li> List</button>";
    }
    ...
    public function renderActions($information,$class="",$edit=TRUE)
    {
        if ($edit)
        {
            $this->renderDeleteCheckbox($information);
            $this->renderEditAction($information);
            $this->renderDeleteAction($information,$this->list_delete_button_next_url,$this->list_delete_button_url);
        } else
        {
            $this->renderViewAction($information);
        }
    }
	...
	public function renderEditControls()
    {
        ...
        $this->renderDeleteAction($this->information,$this->delete_button_next_url,$this->delete_button_url);
   ...
   {
		echo '<img class="icon" src="'.$this->paths->base_url.'/img/displaypng.php?id='.$_SESSION['user_id'].'&image='.$this->paths->user_image.'/informations/'.$information->getId().'_informations_icon.png&key='.$this->current_user->getSecurekey().'" />';
   }
   ...
   public function render_icon_filter()
   {
    ...
    public function renderSearchText($field,$type)
    {
		...
		echo '<input class="search_text '.$type.'" type="text" name="informations_search_texts['.$field.']" value="'.$value.'"></input><button type="button" class="search_text_button clear" ><li class="fa fa-times"></li></button>';
		echo '</div>';
    }
    
    public function get_id($information)
    {
        echo "<button type='button' class='mx-' mx-container='".$this->container."' mx-click='?command=display_rest&base_path=Admin.dir/Informations.dir&path=Edit_Information.hid.rest.php&id=".$information->getId()."&object=Information'  ><i class='fa fa-edit'></i>".$information->getId()."</button>";
    }
